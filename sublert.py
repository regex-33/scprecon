...
def check_new_subdomains(q2): #retrieves new list of subdomains and stores a temporary text file for comparison purposes
    global domain_to_monitor
...
def dns_resolution(new_subdomains): #Perform DNS resolution on retrieved subdomains
    ...
    for domain in subdomains_to_resolve:
        ...
        try:
            ...
            pass
    if dns_results:
        return posting_to_discord(None, True, dns_results) #Discord new subdomains with DNS output
    else:
        return posting_to_discord(None, False, None) #Nothing found notification

def at_channel(): #control discord @channel
    return("<@everyone> " if at_channel_enabled else "")

def posting_to_discord(result, dns_resolve, dns_output): #sending result to discord workplace
    global domain_to_monitor

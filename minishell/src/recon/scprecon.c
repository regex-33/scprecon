...
int main(int argc, char *argv[]) {
    ...
    int listing = 0;
    int reset = 0;
    int remove_flag = 0; // Changed variable name to avoid conflict with remove() function
    ...
    printf("  Reset: %d\n", reset);
    printf("  Discord: %s\n", discord_url);
    printf("  Remove: %d\n", remove_flag); // Changed variable name to avoid conflict with remove() function
	...
	if (remove_flag)
	{
		printf("remove output files\n");
		system("rm -f ./output/file-*.txt");
		exec_command("ls -lah output", 0);
		printf("remove output files\n");
	}
    if (resolve)
